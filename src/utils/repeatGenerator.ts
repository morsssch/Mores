import { RepeatRule } from "../db/repeatRules";
...
export function generateOccurrences(rule: RepeatRule, fromDate = new Date(), days = 30) {
  ...
  if (rule.type === "daily") {
    ...
    const daysOfWeek: number[] = payload.days || [1];
    const time = payload.time || "09:00";
    for (let i = 0; i <= days; i++) {
      const d = new Date(start);
      d.setDate(d.getDate() + i);
    ...
    dates.forEach((s) => {
      ...
      if (!isNaN(d.getTime())) {
        ...
        if (diff >= 0 && diff <= days) occurrences.push(d.toISOString());
      }
    });
  }
