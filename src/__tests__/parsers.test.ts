...
describe('parsers and cleaning', () => {
  ...
  it('parses today and time with colon and removes from text', () => {
    ...
    const pd = parseDateFromText(text);
    const pt = parseTimeFromText(text);
    ...
    const cleaned = cleanTaskText(text, pd?.textMatch, pt?.textMatch);
  ...
  it('does not parse standalone number as time', () => {
    ...
    const cleaned = cleanTaskText(text, undefined, pt?.textMatch);
