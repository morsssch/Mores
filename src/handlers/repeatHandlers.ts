...
import { formatRepeatEntries, repeatSummaryText } from '../utils/repeatHelpers';
...
export function registerRepeatHandlers(bot: Bot<BotContext>) {
    ...
    bot.callbackQuery('set_repeat_daily', async (ctx) => {
        try {
            ...
            await ctx.reply(formatTaskPreview(ctx.session.tempTaskText!, ctx.session.tempDue as string | undefined, ctx.session.tempDueTime, ctx.session.tempReminder) + `\n\nПовторение: каждый день в ${time}`, { reply_markup: (await import('../keyboards')).taskInlineKeyboard });
        } catch (e) {
    ...
    bot.callbackQuery('set_repeat_weekly', async (ctx) => {
        try {
            ...
            await ctx.reply(formatTaskPreview(ctx.session.tempTaskText!, ctx.session.tempDue as string | undefined, ctx.session.tempDueTime, ctx.session.tempReminder) + `\n\nПовторение: каждую ${weekdayName(weekday)} в ${time}`, { reply_markup: (await import('../keyboards')).taskInlineKeyboard });
        } catch (e) {
    ...
    bot.callbackQuery('cancel_repeat', async (ctx) => {
        ...
        } catch (e) { console.error('cancel_repeat', e); try { await ctx.answerCallbackQuery(); } catch(_){} }
    });

    // handle user message when awaiting repeat input
    bot.on('message:text', async (ctx, next) => {
        if (!ctx.session.awaitingRepeatInput) return await next();
        ...
        try {
            ...
            for (const line of lines) {
                ...
                const { parseTimeFromText } = await import('../utils/parseTime');
            ...
            // show parsed schedule and allow saving
            const linesOut = formatRepeatEntries(entries);
            const kb = new InlineKeyboard().text('Сохранить', 'save_repeat').text('Отмена', 'cancel_repeat');
    ...
    bot.callbackQuery('save_repeat', async (ctx) => {
        try {
            ...
            const entries = ctx.session.tempRepeatEntries;
            const summary = repeatSummaryText(entries);
            const preview = (ctx.session.tempTaskText ? formatTaskPreview(ctx.session.tempTaskText, ctx.session.tempDue as string | undefined, ctx.session.tempDueTime, ctx.session.tempReminder) + '\n\n' : '') + (summary || 'Повторения сохранены во временной сессии.');
            await ctx.reply(preview, { reply_markup: (await import('../keyboards')).taskInlineKeyboard });
        } catch (e) { console.error('save_repeat', e); try{ await ctx.answerCallbackQuery(); }catch(_){} }
    });
}

export default registerRepeatHandlers;
