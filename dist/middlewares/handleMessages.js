...
async function handleMessages(ctx) {
    ...
    if (!text)
        return;
    const persistentKeyboard = (0, keyboards_1.getPersistentKeyboard)();
        ...
        try {
            ...
            if (черезЧас) {
                ...
                if (!parsedDate && !parsedTime && !сегодняMatch) {
                    await ctx.reply("Не удалось распознать дату или время. Попробуй ещё раз (например: завтра в 9:00 или 31.10.2025 18:00).", { reply_markup: persistentKeyboard });
                    return;
                }
    ...
    if (ctx.session.awaitingNewTask) {
        await (0, taskHandlers_1.handleNewTaskMessage)(ctx, text);
        return;
    }
    if (ctx.session.tempTaskText) {
        const parsedDate = (0, parseDate_1.parseDateFromText)(text);
        ...
        if (updated) {
            await ctx.reply((0, taskHelpers_1.formatDeadlineUpdate)(ctx.session.tempDue, ctx.session.tempDueTime), { reply_markup: persistentKeyboard });
            return;
        }
    }
    await ctx.reply("Нажми 'Добавить задачу', чтобы начать новую задачу.", {
        reply_markup: persistentKeyboard,
    });
}
